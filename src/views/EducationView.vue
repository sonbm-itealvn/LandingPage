...
<template>
  <MasterLayout>
    <div class="min-h-screen bg-gray-50">
      ...
      <section class="bg-white pt-8 sm:pt-12 lg:pt-16 pb-16 sm:pb-20 lg:pb-24">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-12">
          <div class="mb-8 sm:mb-10 lg:mb-12">
            <h1 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black mb-4 sm:mb-5 lg:mb-6 uppercase tracking-tight text-left text-gray-900 leading-tight">
              ĐÀO TẠO NGÀNH KIẾN TRÚC
            </h1>
            <p class="text-base sm:text-lg lg:text-xl text-gray-700 text-left font-semibold">
          ...
          <div v-else class="relative">
            <!-- Carousel Container -->
            <div class="relative overflow-hidden rounded-lg sm:rounded-xl shadow-lg bg-white" style="min-height: 400px;">
              <div
              ...
              >
                ...
                >
                  <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 p-4 sm:p-6 lg:p-8">
                    <div class="relative h-64 sm:h-80 lg:h-96 rounded-lg overflow-hidden bg-gray-100">
                      <img
                    ...
                    <div class="flex flex-col justify-center p-4 sm:p-6 lg:p-8">
                      <h3 class="text-xl sm:text-2xl lg:text-3xl font-bold mb-3 sm:mb-4 text-gray-900 leading-tight">
                        {{ post.title || 'Bài viết' }}
                      </h3>
                      <p class="text-sm sm:text-base lg:text-lg text-gray-600 mb-4 sm:mb-6 leading-relaxed line-clamp-3">
                        {{ getPostExcerpt(post, 150) }}
                      </p>
                      <RouterLink
                        :to="getPostLink(post)"
                        class="inline-flex items-center text-sm sm:text-base font-semibold hover:underline transition-all"
                        style="color: #1e3a8a;"
                      >
                        Xem chi tiết →
                      </RouterLink>
            ...
            <div class="flex items-center justify-between mt-6 sm:mt-8">
              <!-- Navigation Dots -->
              <div class="flex justify-center items-center gap-2 sm:gap-3 flex-1">
                <button
                  v-for="(post, index) in designPosts"
                  :key="post.id || index"
                  @click="goToSlide(index)"
                  class="w-2 h-2 sm:w-3 sm:h-3 rounded-full transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                  :class="currentSlide === index ? 'bg-yellow-400 scale-125' : 'bg-gray-400 hover:bg-gray-500'"
              ...
              <div class="flex items-center gap-2">
                <button
                  @click="prevSlide"
                  class="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white border-2 border-gray-300 hover:border-gray-400 hover:bg-gray-50 flex items-center justify-center transition-all duration-300 shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
          ...
          <div class="mt-10 sm:mt-12 lg:mt-16">
            ...
            <ul class="space-y-2 sm:space-y-3">
              <li class="flex items-start">
                <span class="text-lg sm:text-xl mr-3 flex-shrink-0" style="color: #3b82f6;">•</span>
      ...
      <section class="bg-gray-50 py-16 sm:py-20 lg:py-24">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-12">
          ...
          <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-10 lg:gap-12">
            <!-- Image Grid (Left) -->
            <div class="grid grid-cols-3 gap-3 sm:gap-4">
              <div
                v-for="(post, index) in theoryPosts.slice(0, 9)"
                :key="post.id || index"
                class="aspect-square rounded-lg overflow-hidden bg-gray-200 relative group cursor-pointer"
              >
                <img
                  v-if="getPostImage(post)"
                  :src="getPostImage(post)"
                  :alt="post.title || 'Bài viết'"
                  class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-300 group-hover:scale-105"
                  loading="lazy"
                />
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-12">
          ...
          <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            <RouterLink
              ...
              :key="post.id || index"
              :to="getPostLink(post)"
              class="group bg-white rounded-lg sm:rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100"
            >
              ...
              <div class="p-4 sm:p-6">
                ...
                <p class="text-sm sm:text-base text-gray-600 mb-3 sm:mb-4 line-clamp-3 leading-relaxed">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 xl:px-12">
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 lg:gap-10">
            <RouterLink
              ...
              :key="post.id || index"
              :to="getPostLink(post)"
              class="group flex flex-col sm:flex-row gap-4 sm:gap-6 bg-white rounded-lg sm:rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100"
            >
              <div class="relative w-full sm:w-64 lg:w-80 h-48 sm:h-64 flex-shrink-0 overflow-hidden bg-gray-100">
                <img
                  :src="getPostImage(post)"
                  :alt="post.title || 'Bài viết'"
                  class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
              ...
              <div class="flex-1 p-4 sm:p-6 flex flex-col justify-center">
                <h3 class="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 text-gray-900 group-hover:text-blue-600 transition-colors leading-tight">
                  {{ post.title || 'Bài viết' }}
                </h3>
                <p class="text-sm sm:text-base text-gray-600 mb-3 sm:mb-4 line-clamp-3 leading-relaxed">
